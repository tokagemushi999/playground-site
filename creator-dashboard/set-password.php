...
if (empty($token)) {
    ...
    $creator = validatePasswordSetToken($db, $token);
    if (!$creator) {
        $error = 'リンクの有効期限が切れているか、既にパスワードが設定されています。';
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $creator) {
    ...
    if (strlen($password) < 8) {
        ...
        if (setCreatorPassword($db, $creator['id'], $password)) {
            $success = true;
        } else {
...
<head>
    ...
    <title>パスワード設定 - <?= htmlspecialchars($siteName) ?></title>
...
<body class="bg-gradient-to-br from-green-50 to-emerald-100 min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md">
        ...
        <div class="bg-white rounded-2xl shadow-xl p-8 text-center">
            ...
            <h2 class="text-xl font-bold text-gray-800 mb-2">パスワードを設定しました！</h2>
            <p class="text-gray-600 mb-6">
                クリエイターダッシュボードにログインできます。
            </p>
        ...
        <div class="bg-white rounded-2xl shadow-xl p-8">
            <div class="text-center mb-6">
                ...
                <img src="/<?= htmlspecialchars($creator['image']) ?>" class="w-20 h-20 rounded-full object-cover mx-auto mb-3">
                <?php else: ?>
                <div class="w-20 h-20 bg-gray-200 rounded-full flex items-center justify-center mx-auto mb-3">
                    <i class="fas fa-user text-gray-400 text-2xl"></i>
                </div>
                <?php endif; ?>
                <h2 class="text-xl font-bold text-gray-800"><?= htmlspecialchars($creator['name']) ?>さん</h2>
                <p class="text-gray-500 text-sm">ようこそ！パスワードを設定してください</p>
            </div>
            ...
            <form method="POST" class="space-y-4">
                ...
                <button type="submit" 
                    ...
                    <i class="fas fa-key mr-2"></i>パスワードを設定
                </button>
            </form>
