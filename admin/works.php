...
// 一括完全削除
if (isset($_POST['bulk_permanent_delete']) && !empty($_POST['selected_works'])) {
    $ids = $_POST['selected_works'];
    $deletedCount = 0;
    
    foreach ($ids as $workId) {
        $workId = (int)$workId;
        ...
        if ($work) {
            ...
            if (!empty($work['back_image']) && file_exists('../' . $work['back_image'])) {
                @unlink('../' . $work['back_image']);
            }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['bulk_sticker_save'])) {
    ...
    if (!$creator_id) {
        ...
        foreach ($titles as $index => $title) {
            ...
            $collectionOrder = $baseOrder + $addedCount + 1;
            $stmt = $db->prepare("INSERT INTO works (creator_id, title, description, category, image, back_image, youtube_url, is_featured, is_omake_sticker, collection_id, collection_order, sort_order, is_manga, reading_direction, view_mode, viewer_theme, first_page_single, crop_position) VALUES (?, ?, '', '', ?, ?, '', 0, 1, ?, ?, 0, 0, 'rtl', 'page', 'dark', 0, '50% 50%')");
            $stmt->execute([$creator_id, $title, $image, $back_image, $collection_id, $collectionOrder]);
            $addedCount++;
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['approval_action'])) {
    if (!validateCsrfToken($_POST['csrf_token'] ?? '')) {
        ...
        if ($action === 'approve') {
            $stmt = $db->prepare("UPDATE works SET approval_status = 'approved', approved_at = NOW(), approval_note = NULL, is_active = 1 WHERE id = ?");
            $stmt->execute([$workId]);
            $message = '作品を承認しました。';
            ...
            if (empty($note)) {
                ...
                $stmt = $db->prepare("UPDATE works SET approval_status = 'rejected', approval_note = ? WHERE id = ?");
                $stmt->execute([$note, $workId]);
                $message = '作品を差し戻しました。';
            }
        }
    }
...
if (isset($_GET['permanent_delete']) && is_numeric($_GET['permanent_delete'])) {
    $workId = (int)$_GET['permanent_delete'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save'])) {
    ...
    $is_line_stamp = isset($_POST['is_line_stamp']);
    // work_typeを判定: LINEスタンプ > ステッカー > 通常
    $work_type = 'normal';
    if ($is_line_stamp) {
        $work_type = 'line_stamp';
        $is_omake_sticker = 0; // LINEスタンプの場合はステッカーフラグをオフ
    } elseif ($is_omake_sticker) {
        $work_type = 'sticker';
    }
    $collection_id = $_POST['collection_id'] ?: null;
    $collection_order = (int)($_POST['collection_order'] ?? 0);
    ...
    if ($id) {
        $stmt = $db->prepare("UPDATE works SET title=?, creator_id=?, description=?, image=?, back_image=?, category=?, tags=?, is_featured=?, is_omake_sticker=?, collection_id=?, collection_order=?, sort_order=?, youtube_url=?, thumbnail_type=?, is_manga=?, reading_direction=?, view_mode=?, viewer_theme=?, first_page_single=?, crop_position=?, work_type=?, product_id=? WHERE id=?");
        $stmt->execute([$title, $creator_id, $description, $image, $back_image, $category, $tags, $is_featured, $is_omake_sticker, $collection_id, $collection_order, $sort_order, $youtube_url, $thumbnail_type, $is_manga, $reading_direction, $view_mode, $viewer_theme, $first_page_single, $crop_position, $work_type, $product_id, $id]);
        $message = '作品を更新しました。';
    } else {
        $stmt = $db->prepare("INSERT INTO works (creator_id, title, description, category, image, back_image, youtube_url, is_featured, is_omake_sticker, collection_id, collection_order, sort_order, is_manga, reading_direction, view_mode, viewer_theme, first_page_single, crop_position, work_type, product_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([$creator_id, $title, $description, $category, $image, $back_image, $youtube_url, $is_featured, $is_omake_sticker, $collection_id, $collection_order, $sort_order, $is_manga, $reading_direction, $view_mode, $viewer_theme, $first_page_single, $crop_position, $work_type, $product_id]);
        $id = $db->lastInsertId();
        $message = '作品を追加しました。';
    }
    ...
    try {
        $workColumns = $db->query("SHOW COLUMNS FROM works")->fetchAll(PDO::FETCH_COLUMN);
        if (in_array('show_in_gallery', $workColumns)) {
            $stmt = $db->prepare("UPDATE works SET show_in_gallery = ?, show_in_creator_page = ?, show_in_top = ? WHERE id = ?");
...
// コレクション一覧を取得
$collections = $db->query("SELECT id, title FROM collections WHERE is_active = 1 ORDER BY sort_order ASC, title ASC")->fetchAll();

// 商品一覧を取得（EC連携用）
$products = [];
try {
    $products = $db->query("SELECT id, name, price, product_type FROM products WHERE is_published = 1 ORDER BY name")->fetchAll();
} catch (PDOException $e) {
    // productsテーブルがない場合は空配列
}
...
try {
    $pendingApprovalCount = $db->query("SELECT COUNT(*) FROM works WHERE approval_status = 'pending'")->fetchColumn();
} catch (PDOException $e) {}
...
?>
        ...
        <div class="flex justify-between items-center mb-8">
            ...
            <div class="flex gap-2">
                ...
                <a href="works.php?filter=pending" class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold px-4 py-3 rounded-lg transition">
        ...
        <div class="bg-yellow-100 border border-yellow-200 text-yellow-700 px-4 py-3 rounded-lg mb-6">
            <i class="fas fa-clock mr-2"></i>審査待ち作品一覧 - 確認後に承認または差し戻ししてください
            <a href="works.php" class="ml-4 text-yellow-800 underline">全て表示</a>
        </div>
        <?php endif; ?>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="flex gap-2 mb-6 border-b border-gray-200 pb-4">
                ...
                <button type="button" onclick="switchTab('store')" id="tab-btn-store" class="tab-btn px-4 py-2 rounded-lg font-bold text-sm bg-gray-100 hover:bg-gray-200 transition">
                    <i class="fas fa-shopping-cart mr-1"></i>ストア連携
                </button>
            </div>
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div id="tab-basic" class="tab-content space-y-6 active">
                    ...
                    $hasWorkDisplaySettings = false;
                    try {
                        $workColumns = $db->query("SHOW COLUMNS FROM works")->fetchAll(PDO::FETCH_COLUMN);
                        $hasWorkDisplaySettings = in_array('show_in_gallery', $workColumns);
                    } catch (PDOException $e) {
                        $hasWorkDisplaySettings = false;
                    }
                    ?>
                    <?php if ($hasWorkDisplaySettings): ?>
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="show_in_gallery" value="1" 
                                       <?= ($editWork['show_in_gallery'] ?? 1) ? 'checked' : '' ?>
                    ...
                    <div class="p-4 bg-purple-50 rounded-lg border border-purple-200">
                        ...
                        <!-- LINEスタンプ設定 -->
                        <div class="flex items-center gap-3 mb-4 p-3 bg-[#06C755]/10 rounded-lg border border-[#06C755]/30">
                            <input type="checkbox" name="is_line_stamp" id="is_line_stamp" 
                                <?= ($editWork['work_type'] ?? '') === 'line_stamp' ? 'checked' : '' ?>
                                onchange="toggleLineStampFields()"
                                class="w-5 h-5 text-[#06C755] rounded focus:ring-[#06C755]">
                            <div>
                                <label for="is_line_stamp" class="text-sm font-bold text-[#06C755] cursor-pointer flex items-center gap-1">
                                    <i class="fab fa-line"></i>LINEスタンプとして表示
                                </label>
                                <p class="text-xs text-gray-500">コレクション内にのみ表示され、トップページやクリエイターページには表示されません</p>
                            </div>
                        </div>
                        
                        <div id="sticker_fields" style="display: <?= ($editWork['is_omake_sticker'] ?? false) || ($editWork['work_type'] ?? '') === 'line_stamp' ? 'block' : 'none' ?>;" class="space-y-4 pt-4 border-t border-purple-200">
                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-2">
                                    コレクション
                                    <a href="collections.php" target="_blank" class="text-xs text-purple-500 hover:underline ml-2">
                                        <i class="fas fa-external-link-alt"></i> コレクション管理
                                    </a>
                                </label>
                                <select name="collection_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-400 outline-none">
                                    <option value="">選択してください</option>
                                    <?php foreach ($collections as $collection): ?>
                                    <option value="<?= $collection['id'] ?>" <?= ($editWork['collection_id'] ?? '') == $collection['id'] ? 'selected' : '' ?>>
                                        <?= htmlspecialchars($collection['title']) ?>
                                    </option>
                                    <?php endforeach; ?>
                                </select>
                                <p class="text-xs text-gray-500 mt-1">コレクションに属さない場合は、単独ステッカーとして表示されます</p>
                            </div>
                            
                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-2">コレクション内表示順</label>
                                <input type="number" name="collection_order" value="<?= $editWork['collection_order'] ?? 0 ?>" 
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-400 outline-none">
                                <p class="text-xs text-gray-500 mt-1">コレクション内での表示順序（数字が小さいほど前に表示）</p>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-2 gap-3 sm:gap-4">
                                    ...
                                    <div class="bg-white rounded-lg border-2 border-gray-200 p-2 sm:p-3">
                                        ...
                                        <p class="text-xs text-gray-500 mt-1 text-center hidden sm:block">推奨: 1000×1000px</p>
                                    </div>
                                    ...
                                    <div class="bg-white rounded-lg border-2 border-purple-200 p-2 sm:p-3">
                                        ...
                                        <p class="text-xs text-purple-600 mt-1 text-center hidden sm:block">推奨: 1000×1000px</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div id="image_field">
                        ...
                        <div class="flex gap-4 items-start">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-xs text-gray-500 mt-1">推奨: <strong>1000×1000px</strong>（正方形）または <strong>800×1200px</strong>（縦長）</p>
                            </div>
                        </div>
                    </div>
                ...
                <div id="tab-store" class="tab-content space-y-6">
                    <div class="p-4 bg-green-50 rounded-lg border border-green-200">
                        <label class="block text-sm font-bold text-green-800 mb-3">
                            <i class="fas fa-shopping-cart mr-1"></i>販売商品との連携
                        </label>
                        <p class="text-xs text-green-700 mb-4">
                            この作品を商品と連携させると、作品詳細に「購入する」ボタンが表示されます。<br>
                            漫画の場合は、購入者のみ全ページを閲覧可能にできます。
                        </p>
                        
                        <?php if (empty($products)): ?>
                        <div class="bg-white rounded-lg p-4 text-center">
                            <p class="text-gray-500 mb-3">まだ商品が登録されていません</p>
                            <a href="products.php?new=1" class="inline-block px-4 py-2 bg-green-500 text-white rounded-lg text-sm font-bold hover:bg-green-600 transition">
                                <i class="fas fa-plus mr-1"></i>商品を作成
                            </a>
                        </div>
                        <?php else: ?>
                        <select name="product_id" class="w-full px-4 py-2 border border-green-300 rounded-lg focus:ring-2 focus:ring-green-400 outline-none bg-white">
                            <option value="">連携しない</option>
                            <?php foreach ($products as $product): ?>
                            <option value="<?= $product['id'] ?>" <?= ($editWork['product_id'] ?? '') == $product['id'] ? 'selected' : '' ?>>
                                <?= htmlspecialchars($product['name']) ?> - ¥<?= number_format($product['price']) ?>
                                (<?= $product['product_type'] === 'digital' ? 'デジタル' : ($product['product_type'] === 'physical' ? '物販' : 'セット') ?>)
                            </option>
                            <?php endforeach; ?>
                        </select>
                        <p class="text-xs text-green-600 mt-2">
                            <i class="fas fa-info-circle mr-1"></i>
                            連携した商品を購入した会員は、本棚からこの作品を閲覧できます
                        </p>
                        <?php endif; ?>
                    </div>
                    
                    <?php if (($editWork['is_manga'] ?? 0) || !empty($mangaPages)): ?>
                    <div class="p-4 bg-purple-50 rounded-lg border border-purple-200">
                        <label class="block text-sm font-bold text-purple-800 mb-2">
                            <i class="fas fa-eye mr-1"></i>試し読み設定
                        </label>
                        <p class="text-xs text-purple-700 mb-3">
                            商品と連携した場合、未購入者には以下のページ数まで表示されます
                        </p>
                        <div class="flex items-center gap-3">
                            <input type="number" name="preview_pages" value="<?= $editWork['preview_pages'] ?? 3 ?>" min="0" max="100"
                                class="w-24 px-3 py-2 border border-purple-300 rounded-lg focus:ring-2 focus:ring-purple-400 outline-none">
                            <span class="text-sm text-purple-700">ページまで試し読み可能</span>
                        </div>
                        <p class="text-xs text-purple-600 mt-2">
                            0を設定すると試し読み不可になります
                        </p>
                    ...
                    <div class="p-4 bg-gray-50 rounded-lg">
                        <h4 class="font-bold text-gray-700 mb-2">
                            <i class="fas fa-question-circle mr-1"></i>ストア連携について
                        </h4>
                        <ul class="text-xs text-gray-600 space-y-1">
                            <li>• 作品詳細モーダルに「購入する」ボタンが表示されます</li>
                            <li>• 漫画ビューアーで試し読みページ以降は「購入が必要」と表示されます</li>
                            <li>• 購入者は本棚からいつでも作品を閲覧できます</li>
                            <li>• 商品の価格や説明は<a href="products.php" class="text-green-600 hover:underline">商品管理</a>で設定します</li>
                        </ul>
                    </div>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6 mb-6">
            ...
            <form method="POST" enctype="multipart/form-data" id="bulk-sticker-form">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    ...
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">コレクション（任意）</label>
                        <select name="bulk_collection_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-400 outline-none">
                            <option value="">選択してください</option>
                            <?php foreach ($collections as $collection): ?>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="p-4 border-b flex gap-2 bg-gray-50">
                    <?php if ($showArchived): ?>
                        ...
                        <button type="submit" name="bulk_permanent_delete" class="bg-red-500 text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-red-600 disabled:opacity-50 transition" disabled id="permanent-delete-btn" onclick="return confirm('選択した作品を完全に削除しますか？\n\nこの操作は取り消せません。\n画像ファイルも削除されます。')">
                            <i class="fas fa-trash-alt mr-1"></i>選択を完全削除
                        </button>
                    <?php else: ?>
                    ...
                    <tbody>
                        ...
                        <tr class="border-t hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-center">
                                ...
                                <?php else: ?>
                                    <a href="?permanent_delete=<?= $work['id'] ?>" onclick="return confirm('完全に削除しますか？\n\nこの操作は取り消せません。\n画像ファイルも削除されます。')" class="text-red-400 hover:text-red-600" title="完全削除">
    ...
    <script>
        ...
        function toggleStickerFields() {
            ...
            const isLineStampEl = document.getElementById('is_line_stamp');
            const stickerFields = document.getElementById('sticker_fields');
            const imageField = document.getElementById('image_field');
            // 要素が存在する場合のみ実行
            if (stickerFields) {
                const isSticker = isStickerEl && isStickerEl.checked;
                const isLineStamp = isLineStampEl && isLineStampEl.checked;
                stickerFields.style.display = (isSticker || isLineStamp) ? 'block' : 'none';
                // ステッカー時は通常の画像フィールドを非表示（ステッカー内に表面画像があるため）
                if (imageField) {
                    imageField.style.display = (isSticker || isLineStamp) ? 'none' : 'block';
                }
            }
        }
        
        // LINEスタンプフィールド切り替え
        function toggleLineStampFields() {
            const isLineStampEl = document.getElementById('is_line_stamp');
            const isStickerEl = document.getElementById('is_omake_sticker');
            // LINEスタンプにチェックを入れたら、ステッカーのチェックを外す
            if (isLineStampEl && isLineStampEl.checked && isStickerEl) {
                isStickerEl.checked = false;
            }
            toggleStickerFields();
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateBulkButton() {
                ...
                if (permanentDeleteBtn) {
                    permanentDeleteBtn.disabled = checked === 0;
                }
            }
