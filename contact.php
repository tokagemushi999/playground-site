...
// データベースに保存
try {
    // service_idカラムが存在するか確認
    $hasServiceId = false;
    try {
        $columns = $db->query("SHOW COLUMNS FROM inquiries")->fetchAll(PDO::FETCH_COLUMN);
        $hasServiceId = in_array('service_id', $columns);
    } catch (Exception $e) {
        $hasServiceId = false;
    }
    
    if ($hasServiceId && $service_id) {
        $stmt = $db->prepare("
            INSERT INTO inquiries 
            (nominated_creator, genre, budget, deadline, purpose, details, name, email, company_name, project_name, usage_permission, service_id, status) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'new')
        ");
        $stmt->execute([
